...
export const handleFormChange = (e: any, formValue: any, setFormValue: any) => {
  e.preventDefault();
...
export const videoCheck = (file: any) => {
  let error = "";
  if (!file) {
    return (error = "No files selected!");
  }
...
export const fileCheck = (file: any) => {
  let error = "";
...
export const addFile = (addedFiles: any, setContent: any) => {
  const files = addedFiles.map((file: any) =>
...
export const handleCreatePost = async (content: any, createPost: any) => {
  if (
...
export const handleEditPost = async (
  ...
  curContent.images =
    updatedImageList.length > 0
      ? [
          ...curContent.images.filter((item: any) => !item.preview),
          ...updatedImageList,
        ]
      : curContent.images;
  curContent.videos =
    updatedVideoList.length > 0
      ? [
          ...curContent.videos.filter((item: any) => !item.preview),
          ...updatedVideoList,
        ]
      : curContent.videos;
...
export const checkType = (item: any, addCl: any) => {
  if (typeof item !== "string") {
    if (item?.type?.startsWith("image")) {
      return (
        <img
...
export const handleCreateComment = async (
  ...
  if (e.key === "Enter" && !e.shiftKey) {
    ...
    if (commentContent.image?.preview) {
      var imageUrl: any = await uploadFile([commentContent.image]);
    }
    if (commentContent.video?.preview) {
      var videoUrl: any = await uploadFile([commentContent.video]);
    }
...
export const handleLikeComment = async (
  ...
  if (!cnd) {
    ...
    let updatedItem = {
      ...commentList[pos],
      likes: commentList[pos].likes.filter((cmt: any) => cmt !== user._id),
    };
    setCommentList((state: any) => [
      ...state.slice(0, pos),
      updatedItem,
